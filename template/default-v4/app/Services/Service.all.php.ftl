...
class ${pojo.name}${role.name?cap_first}Service
{
    ...
    public function findBy${fieldGeneric.name?cap_first}Id($id) {
        return ${pojo.name}::where('${fieldGeneric.name?uncap_first}_id', $id)->get();
    }
    ...
    public function findWithAssociatedLists($id) {
        return ${pojo.name}::with([<#list pojo.fields as field><#if field.list>'${field.name?uncap_first}',</#if></#list>])->find($id);
    }
    </#if>

    <#if pojo.labelOrReference??>
    public function findAllOptimized() {
        return ${pojo.name}::select('id', '${pojo.labelOrReference.name}')->get();
    }
    </#if>

    public function findById($id) {
        return ${pojo.name}::find($id);
    }
}
